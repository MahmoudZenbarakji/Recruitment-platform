...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.applicatnt.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.applicatnts.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('education_level') ? 'is-invalid' : '' }}" name="education_level" id="education_level" required>
                    ...
                    @foreach(App\Models\Applicatnt::EDUCATION_LEVEL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('education_level', 'Associate') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('experience_year') ? 'is-invalid' : '' }}" type="text" name="experience_year" id="experience_year" value="{{ old('experience_year', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cvs') ? 'is-invalid' : '' }}" name="cvs[]" id="cvs" multiple required>
                    @foreach($cvs as $id => $cv)
                        <option value="{{ $id }}" {{ in_array($id, old('cvs', [])) ? 'selected' : '' }}>{{ $cv }}</option>
                    @endforeach
                </select>
                @if($errors->has('cvs'))
